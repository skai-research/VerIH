...
torchrun --rdzv-backend=c10d --rdzv-endpoint=localhost:0 --nproc_per_node 8 torchllms/training/trainers/dpo.py \
    ...
    --output_dir outputs/system/final-dpo-${BASE}_lr=${LR}_epochs=${EPOCHS}_beta=${BETA}_smoothing=${SMOOTHING}_seed=${SEED}