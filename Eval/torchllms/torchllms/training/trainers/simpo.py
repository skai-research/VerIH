...
def add_dpo_args(parser):
    parser.add_argument(
        "--simpo_beta", type=float, default=1.0, help="Margin scaling factor in loss."
    )
    parser.add_argument(
        "--simpo_gamma",
        type=float,
        default=1.0,
        help="Target margin size in loss. Re-defined from paper to be unit-scaled.",
    )
    return parser
...
def main():
    ...
    assert not args.pack_samples, "Packing samples not supported for SimPO"
    ...
    for epoch in range(args.train_epochs):
        ...
        for batch in train_dataloader:
            ...
            for micro_batch in micro_batches:
                ...
                length_w = (labels_w != data.IGNORE_ID).sum(1)
                length_l = (labels_l != data.IGNORE_ID).sum(1)
                ...
                margin = logprobs_w / length_w - logprobs_l / length_l
                loss = args.simpo_beta * (margin - args.simpo_gamma)
                loss = -F.logsigmoid(loss).mean()

                acc = (margin > 0).float().mean()
                global_accuracy += acc / args.gradient_accum_steps
