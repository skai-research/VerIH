...
from torchllms.inference.generate import generate_response
...
def inference_with_vllm(
    ...
    # generate_response(self.model, {"temperature": self.temperature}, prompts)

    assert do_chat_formatting == False
    assert tokenizer == None
    assert return_full_outputs == False
    ...
    if is_generation:
        outputs = generate_response(model, {"temperature": temperature}, prompts)
        results = [conv[-1]["content"] for conv in outputs]

        # Save Reasoning traces
        output_path = Path(f"./outputs/{os.getenv('OUTPUT_FOLDER')}") / f"{os.getenv('VARIANT')}_response.jsonl"
        output_path.parent.mkdir(parents=True, exist_ok=True)
        logs = [{"messages":x} for x in outputs]
...
def create_and_inference_with_vllm(
    ...
    if is_generation:
        llm = providers.PROVIDERS["vllm"](model_path=model_name_or_path, tensor_parallel_size=int(os.getenv("NUM_GPU", 1)), total_tokens=10240, response_tokens=8192)
    else:
...
def subprocess_inference_with_vllm(
    ...
    subprocess = ctx.Process(
        ...
        args=(
            ...
            return_full_outputs, temperature, top_p, max_tokens, n, stop, is_generation, child_conn
        ),
