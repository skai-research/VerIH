...
from utils.run_vllm_model import prepare_tool_call
import task_execution as task_eval
import safety as safety_eval
import rule_following as rule_eval
import tool_use as tool_eval
...
def main():
    ...
    parser.add_argument(
        "-backend", type=str, choices=["vllm", "api", "torchllms"], required=True
    )
    ...
    if args.backend == "torchllms": # save jsonl
        for example in requests:
            messages = example['messages']
            if "tool" in example.keys():
                _, tool_call, tool_return = prepare_tool_call(example["tool"])
                messages.extend([tool_call, tool_return])

        with jsonlines.open(args.request_file, mode="w") as writer:
    ...
    if args.backend == "torchllms":
        inference_command = [
            "python", "-m", "torchllms.inference.generate",
            "--input_path", args.request_file,
            "--output_path", args.response_file,
            "--provider", "vllm",
            "--provider_kwargs", json.dumps({
                "model_path": args.model,
                "tensor_parallel_size": args.tensor_parallel,
                "total_tokens": 10240,
                "response_tokens": 8192,
            }),
            "--generate_kwargs", json.dumps({"temperature": 0.0}),
        ]

        print(colored("-" * 20 + "Inference" + "-" * 20, "green"))
        print(inference_command)
        subprocess.run(inference_command, check=True)
        with jsonlines.open(args.response_file) as reader:
